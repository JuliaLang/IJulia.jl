...
# If you're running the tests locally you could comment out the
# JULIA_CONDAPKG_* lines above and uncomment the two environment variables
# below. This will be a bit faster since it stops CondaPkg from re-resolving the
# environment each time (but you do need to run it at least once locally to
# initialize the `@ijulia-tests` environment).
